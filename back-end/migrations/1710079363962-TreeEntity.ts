...
export class TreeEntity1710079363962 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`note_closure\` ADD CONSTRAINT \`FK_c595484fba68bc88524030ce370\` FOREIGN KEY (\`id_ancestor\`) REFERENCES \`note\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`note_closure\` ADD CONSTRAINT \`FK_924868788a237a99f9bb21e53ba\` FOREIGN KEY (\`id_descendant\`) REFERENCES \`note\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
    }
