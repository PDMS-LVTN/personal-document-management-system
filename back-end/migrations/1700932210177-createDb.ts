...
export class CreateDb1700932210177 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`note\` (\`id\` varchar(36) NOT NULL, \`title\` longtext NOT NULL, \`content\` longtext NULL, \`size\` int NOT NULL, \`read_only\` tinyint NOT NULL DEFAULT 0, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`parent_id\` varchar(36) NULL, \`user_id\` varchar(36) NOT NULL, PRIMARY KEY (\`id\`), FULLTEXT (\`content\`), FULLTEXT(\`title\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
